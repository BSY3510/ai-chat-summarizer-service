...
- **대화방(Session) 관리 (CRUD)**: 여러 번의 대화를 하나의 방으로 묶고, 대화방 생성/조회/제목 수정 및 삭제 기능 완비
...
### 3. 양방향 순환 참조 해결 및 DTO 패턴 도입
- **문제**: JPA의 1:N 양방향 매핑(`ChatSession` - `ChatMessage`) 구조에서, Entity를 그대로 프론트엔드에 반환하려다 보니 JSON 직렬화 과정에서 무한 참조 에러(`StackOverflowError`)가 발생함.
- **해결**: `@JsonIgnore`와 같은 뷰 템플릿 종속적인 임시 조치를 지양하고, API 응답 전용 객체인 **DTO(Data Transfer Object)** 패턴을 전면 도입함. 이를 통해 API 스펙과 도메인 모델(Entity)의 결합도를 낮추고 순환 참조를 아키텍처 레벨에서 원천 차단함.
